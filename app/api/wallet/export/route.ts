...
/**
 * Export wallet private key and recovery phrase
 * POST /api/wallet/export
 * Body: { walletId: string, password?: string }
 * 
 * SECURITY: This endpoint requires authentication and password verification
 */
  ...
  try {
    ...
    if (!walletId) {
      return NextResponse.json(
        { error: 'Wallet ID is required', success: false },
        { status: 400 }
      );
    }

    // Get encrypted key from cookie
    const cookieName = `wallet_${walletId}`;
    const encryptedKey = request.cookies.get(cookieName)?.value;

    if (!encryptedKey) {
      // Try to get from database if email-based wallet
      // This would require email in the request or session
      return NextResponse.json(
        { error: 'Wallet not found. Please ensure you are logged in.', success: false },
        { status: 404 }
      );
    }
    ...
    try {
      secretKey = decryptWalletKey(encryptedKey);
    } catch (error) {
      return NextResponse.json(
        { error: 'Failed to decrypt wallet. Invalid session or corrupted data.', success: false },
        { status: 401 }
      );
    }
    ...
    // Generate recovery phrase from seed
    // Note: Solana keypairs don't natively use BIP39
    // We'll create a simple word-based representation from the seed
    // This is NOT a standard BIP39 phrase but provides a human-readable backup
    ...
    const wordList = [
      ...
      'example', 'exceed', 'excel', 'exception', 'exchange', 'excite', 'exclude', 'excuse', 'execute', 'exercise',
      ...
      'peace', 'peach', 'peacock', 'peak', 'pear', 'peasant', 'pelican', 'pen', 'penalty', 'pencil',
      ...
      'pipe', 'pistol', 'pitch', 'pizza', 'place', 'planet', 'plastic', 'plate', 'play', 'playground',
      'pleasant', 'please', 'pledge', 'pluck', 'plug', 'plunge', 'poem', 'poet', 'point', 'polar',
      ...
      'red', 'reduce', 'refer', 'reflect', 'reform', 'refuse', 'region', 'regret', 'regular', 'reject',
      ...
      'ring', 'riot', 'rip', 'ripe', 'rise', 'risk', 'rival', 'river', 'road', 'roast',
      ...
      'sock', 'soda', 'soft', 'solar', 'soldier', 'solid', 'solve', 'someone', 'song', 'soon',
      ...
      'sunny', 'sunset', 'super', 'supply', 'support', 'sure', 'surface', 'surge', 'surprise', 'surround',
      ...
      'wash', 'wasp', 'waste', 'water', 'wave', 'way', 'wealth', 'weapon', 'weary', 'weather',
      'weave', 'web', 'wedding', 'weekend', 'weird', 'welcome', 'west', 'wet', 'whale', 'what',
    ...
    // Generate 12-word phrase from seed
    const words: string[] = [];
    for (let i = 0; i < 12; i++) {
      const byte1 = seed[i * 2] || 0;
      const byte2 = seed[i * 2 + 1] || 0;
      const index = (byte1 * 256 + byte2) % wordList.length;
      words.push(wordList[index]);
    }
    const recoveryPhrase = words.join(' ');

    // Convert private key to base58 (Solana standard)
    ...
    return NextResponse.json({
      ...
      warning: 'Keep this information secure. Anyone with access can control your wallet.',
    });
  } catch (error) {
