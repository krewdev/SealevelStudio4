// Next.js API route to proxy Birdeye pair transaction seek_by_time requests
// Fetches pair (pool) transactions sorted by time
...
export async function GET(request: NextRequest) {
  try {
    ...
    const addressValidation = validateSolanaAddress(pairAddress);
    if (!addressValidation.valid) {
      return NextResponse.json(
        { error: addressValidation.error || 'Invalid pair address format' },
        { status: 400 }
      );
    }
    ...
    params.append('address', pairAddress);
    ...
    const url = `${BIRDEYE_API_BASE}/defi/txs/pair/seek_by_time?${params.toString()}`;
    ...
    console.error('Birdeye pair transactions proxy error:', error);
    return NextResponse.json(
