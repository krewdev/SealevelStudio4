/**
 * Simple mnemonic phrase generator for Solana wallets
 * Note: This generates a 12-word mnemonic from a seed
 * For production, consider using a proper BIP39 library
 */

// Common word list (simplified - in production use BIP39 word list)
const WORD_LIST = [
  ...
  'shoot', 'shop', 'short', 'shoulder', 'shove', 'shovel', 'show', 'shrimp',
  ...
  'stain', 'stair', 'stake', 'stale', 'stamp', 'stand', 'start', 'state',
...
/**
 * Generate a 12-word mnemonic phrase from a seed
 */
export function generateMnemonic(seed: Uint8Array): string {
  const words: string[] = [];
  for (let i = 0; i < 12; i++) {
    const index = (seed[i] + (seed[i + 12] || 0) + (seed[i + 24] || 0)) % WORD_LIST.length;
...
/**
 * Generate a mnemonic from a Solana keypair secret key
 */
export function generateMnemonicFromKeypair(secretKey: Uint8Array): string {
  // Use the secret key as seed
  return generateMnemonic(secretKey);
}
