...
export function snapshotToPrompt(snap: PageSnapshot): string {
  return [
    ...
    `PREFLIGHT_VENUE=${snap.session.lastPreflight?.venue || 'none'}`,
    `PREFLIGHT_WORST_CASE_SOL=${snap.session.lastPreflight?.worstCaseSol ?? 'n/a'}`,
    `PREFLIGHT_BLOCKERS=${snap.session.lastPreflight?.blockers ?? 'n/a'}`,
